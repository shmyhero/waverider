...
class RollYield(object):
    ...
    def condition_z_score(self, ma_window_short=7, ma_window_long=20):
        ...
        zscore = map(lambda x, y, z: (x-y)/z, rate_ma_short[ma_window_long:], rate_ma_long[ma_window_long:], rate_std[ma_window_long:])
        print zscore
        return map(lambda x: x > 0.8, zscore)

    def get_returns(self, condition, ma_window, print_trade_node = False):
        trade_nodes = []
        previous_condition = False
        conditions = condition(7, ma_window)
    ...
    def run1(self, ma_window = 7, print_trade_node=True):
        returns = self.get_returns(self.condition_z_score, ma_window, print_trade_node)
        self.plot(returns)
